...
export default function FreelancerAccount() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      if (!uid) {
        navigate("/account/freelancer-login");
        return;
      }

      try {
        const docSnap = await getDoc(doc(db, "freelancers", uid));
        if (docSnap.exists()) {
        ...
        <div className="grid gap-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <p className="text-sm text-gray-600">Почасовая ставка</p>
                <p className="font-medium">€{profile.hourlyRate}/час</p>
              </div>
              {profile.description && (
                <div>
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-gray-600">
                Здесь будут отображаться ваши текущие заказы от клиентов.
              </p>
            </CardContent>
          </Card>
        </div>
